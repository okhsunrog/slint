...
The backend preserves libinput's device defaults, including the acceleration profile and tap-to-click setting.
Set `SLINT_LIBINPUT_ACCEL_SPEED` to a number from `-1.0` to `1.0` to override pointer acceleration speed.
For example, `SLINT_LIBINPUT_ACCEL_SPEED=0.3` increases speed; `0.0` is neutral.
Set `SLINT_LIBINPUT_TAP_TO_CLICK=1` to enable tapping on touchpads, or `0` to disable it.
Unset or invalid values leave the corresponding device setting unchanged.
These settings are read when the event loop starts and also apply to devices connected later.

## Configuring the Keyboard

Without libseat, Slint disables keyboard translation on the active Linux virtual console while the event loop runs.
Queued input is flushed before the original keyboard mode is restored, so GUI input cannot become shell commands.
Initialization fails if an existing virtual console cannot be protected.

The original mode is restored when the event loop returns or unwinds.
Process aborts and termination by a signal, including `SIGTERM` or `SIGKILL`, bypass this cleanup and can leave the console keyboard disabled.
Use libseat for session management, or run the application under a supervisor that restores the console after a crash.
